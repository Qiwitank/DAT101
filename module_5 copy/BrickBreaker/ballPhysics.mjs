...
import lib2D from "../../../DAT101/common/libs/lib2d_v2.mjs";
...
class TBallPhysics {
  ...
  // Bruk denne koden til å sjekke om ballen kolliderer med bricks
  // Hint: gå inn i Ball.mjs og se på hvordan vi sjekker om ballen kolliderer med veggen
  #collidedWithBrick(aBrick) {
    if (this.#sprite.x + this.#sprite.width > aBrick.x && this.#sprite.x < aBrick.x + aBrick.width && this.#sprite.y + this.#sprite.height > aBrick.y && this.#sprite.y < aBrick.y + aBrick.height) {
      if (this.#sprite.y + this.#sprite.height > aBrick.y && this.#sprite.y < aBrick.y + aBrick.height) {
        this.#directionVector.y *= -1;
      }
      if (this.#sprite.x + this.#sprite.width > aBrick.x && this.#sprite.x < aBrick.x + aBrick.width) {
        this.#directionVector.x *= -1;
      }
      this.#speedVector.calculateMovement(this.#directionVector, this.#speed);
      return true;
  ...
  update(aBounds, aHero, aBrick) {
    ...
    for(let i = 0; i < aBrick.length; i++){
      ...
      if(this.#collidedWithBrick(aBrick[i])) {  
